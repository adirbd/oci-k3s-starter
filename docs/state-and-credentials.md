...
The most urgent: this token controls your Cloudflare tunnel and DNS routes.

1. Create a new token at <https://one.dash.cloudflare.com> → Networks → Tunnels → your
   tunnel → Configure → Token.
2. Update the vault entry (console → Vault → Secrets → `k3s-01-cloudflared-token`, or the
   CLI). With rung 4 enabled, External Secrets picks up the new value on its next refresh
   cycle (default: 1 hour). To force it immediately:
   ```bash
   kubectl -n cloudflared delete secret cloudflared-token
   # ExternalSecret recreates it within seconds
   ```
3. Optionally revoke the old token in the Cloudflare dashboard.

### Grafana admin password

Generated by Terraform and delivered as a Secret (or ExternalSecret with rung 4).

- **Easiest:** change it in the cluster directly:
  ```bash
  ...
  ```
  Update `terraform.tfvars` or re-apply so Terraform state stays in sync.

- **Nuclear:** `tofu taint` the password resource and re-apply. This generates a brand new
  password and overwrites the Secret.
...
```bash
tofu taint tls_private_key.console
tofu apply
```
...
The Customer Secret Key used by the `s3` backend. Rotate via the OCI console (Identity →
Users → your user → Customer secret keys → delete the old one), then regenerate:

```bash
tofu taint oci_identity_customer_secret_key.state
tofu apply
```

Update your environment variables to match the new outputs.
